...
public class CheckpointMechanic implements Mechanic {
    ...
    public void setup(Context context) {
        ...
        Map<Point, Boolean> blocks = context.mechanic(BlockAnalysisMechanic.class).PRESSURE_PLATES.get();
        List<Point> spawnPlates = new ArrayList<>();

        // get spawn points
        for (Entry<Point, Boolean> plate : blocks.entrySet()) {
            if (plate.getValue()) {
        ...
        context.events().addListener(PlayerMoveEvent.class, (event) -> {
            ...
            for (Entry<Point, Point> plate : checkpoints.entrySet()) {
                ...
                if     (platePos.blockX() == pos.blockX() && platePos.blockZ() == pos.blockZ() &&
                    ...
                    if (event.getPlayer().getRespawnPoint().distanceSquared(plate.getValue()) > 0.5) {
                        event.getPlayer().setRespawnPoint(Pos.fromPoint(plate.getValue()).withYaw(-90.0F));
                        event.getPlayer().sendActionBar(Component.text("Checkpoint Reached!"));
                    }
                    break;
                }
            }
